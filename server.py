...
# AV-HuBERT/fairseq is optioneel (zie configs/default.yaml ->
# secondary_ensemble.enabled) en NIET meer in requirements.txt opgenomen —
# fairseq's oude dependency-eisen (omegaconf<2.1, antlr4==4.8) botsen met de
# rest van deze stack. Deze import is daarom defensief: als de module of
# fairseq zelf ontbreekt, start de API gewoon door zonder het secundaire
# model, in plaats van hard te crashen bij opstarten.
try:
    from models.av_hubert_wrapper import AVHubertTranscriber
except ImportError:
    AVHubertTranscriber = None
...
# CORS: allows the JS web UI to call this API. This deployment (see
# deploy.sh) serves the API on 0.0.0.0:6040 and the web UI on 0.0.0.0:6041,
# i.e. both reachable from other machines on the network — not localhost-only
# as the original repo README describes. allow_origin_regex accepts any host
# on port 6041 (the web UI's port) since we don't know every client IP in
# advance; specific localhost/127.0.0.1 origins are kept for local testing.
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        "http://localhost",
    ...
    allow_origin_regex=r"http://.*:6041$",
...
def load_everything():
    ...
    if cfg["models"]["secondary_ensemble"]["enabled"]:
        if AVHubertTranscriber is None:
            raise RuntimeError(
                "secondary_ensemble.enabled staat op true in configs/default.yaml, "
                "maar fairseq is niet geïnstalleerd (bewust verwijderd uit "
                "requirements.txt, zie het commentaar daar). Installeer fairseq "
                "handmatig in een apart venv, of zet secondary_ensemble.enabled "
                "terug op false."
            )
...
if __name__ == "__main__":
    import uvicorn
    # Host 0.0.0.0: see deploy.sh — this deployment is intentionally exposed
    # to the network on port 6040, not localhost-only.
