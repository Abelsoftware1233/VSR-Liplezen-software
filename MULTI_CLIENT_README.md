...
> **Production deployment note:** `deploy.sh` in the repo root sets up the
> API and web UI as systemd services on **0.0.0.0:6040** (API) and
> **0.0.0.0:6041** (web UI) — i.e. reachable over the network, not
> localhost-only. This is a deliberate deviation from the manual setup
> described below, which stays localhost-only as originally designed. See
> the warning banner at the top of `deploy.sh` before using it.
>
> **Also note:** the secondary AV-HuBERT/fairseq ensemble model is disabled
> in this deployment (`secondary_ensemble.enabled: false` in
> `configs/default.yaml`, and fairseq is commented out of `requirements.txt`).
> fairseq's dependency requirements are incompatible with the rest of this
> stack in a single Python environment (see the comment in `requirements.txt`
> for details). Only the primary Auto-AVSR model runs. `api/server.py`
> imports `AVHubertTranscriber` defensively, so this doesn't affect the
> primary transcription pipeline.

**Manual setup below runs 100% locally.** The "API" is a Python process on
